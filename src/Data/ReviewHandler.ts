...
export class ReviewHandler {
  ...
  public async fetchReviewsFromBackend(): Promise<void> {
    try {
      const response = await fetch(
        "https://localhost:3000/api/reviewsRoute/getReviews"
      );
  ...
  async addReviewsToDatabse(newReview: Reviews) {
    fetch("https://localhost:3000/api/reviewsRoute/addReviews", {
      method: "POST",
      headers: {
