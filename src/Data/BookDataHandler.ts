...
export class BookDataHandler {
  ...
  countBookingsBySession(bookings: Booking[]): Map<number, number> {
    const countMap = new Map<number, number>();

    for (const booking of bookings) {
        // Get the session number and convert it to an integer
        const session = booking.session;

        // If the session is already a key in the map, increment its value
        // Otherwise, add it to the map with a value of 1
        if (countMap.has(session)) {
            const currentCount = countMap.get(session);
            if (currentCount !== undefined) {
                countMap.set(session, currentCount + 1);
            }
        } else {
            countMap.set(session, 1);
        }
    }
